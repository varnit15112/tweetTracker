...
chrome.runtime.onMessage.addListener(function(request, sender) {
  if (request.action == "getSource") {
      //console.log('ret='+request.source);
      message = request.source;
  }
});

Firebase.enableLogging(true);
var fr= new Firebase("https://tweettracker-3d814.firebaseio.com/allTweets");
var fr2= new Firebase("https://tweettracker-3d814.firebaseio.com/Users");
    
function onWindowLoad() {
    //document.getElementById("button1").addEventListener("click",sendKeywords);
    // chrome.storage.sync.get('words',function(result){
    //   document.getElementById("t1").innerHTML=result.words;
    // });
	console.log("hi");
	...
	try{
		...
		fr2.once("value", function(snap) {
			var allU = snap.val();
			var uNum = snap.numChildren();

			console.log("**inside**");  //should only run once


			for (var key in allU){
			  	var singleU = allU[key];
				var Un = singleU["email"]			  	

				if (Un==currUser){
					currList=singleU["list"];
	...
	//document.getElementById("textTweets").innerHTML = "click";

	var allText = new Array();
	var allImage = new Array();
	var allTextandImage = new Array();

	

	try{

		fr.once("value", function(snap) {
			var allT = snap.val();
			var uNum = snap.numChildren();
			...
			for (var key in allT){
			  	var singleT = allT[key];

			  	var hasText=0;
			  	var hasImg=0
			  	
			  	if (singleT.hasOwnProperty('entities')){
			  		var tEnt = singleT["entities"];
			  		if(tEnt.hasOwnProperty('media')){
			  			hasImg=1;
					}
			  	}


			  	var text = singleT["text"];
			  	...
			  	if (text){
				  	...
				  	var uName = singleT["user"]["screen_name"];
				  	...
				  	flag3=0;
				  	for (var li in currList){
				  		if (li.localeCompare(uName)){
				  			flag3=1;
				  		}
				  	}
				  	...
				  	if (flag3==1){
				  		...
				  		if (hasText==1 && hasImg==1){
					  		allTextandImage.push(text);
					  	}
					  	else if (hasText==0 && hasImg==1){
					  		allImage.push(text);
					  	}else if (hasText==1 && hasImg==0){
					  		allText.push(text);
					  	}
		  	...
		  	console.log(allTextandImage);

		  	document.getElementById("textTweets").innerHTML = allText;
		  	document.getElementById("imageTweets").innerHTML = allImage;
		  	document.getElementById("textAndImageTweets").innerHTML = allTextandImage;
...
{
	try{
	    // if(document.keywordform.words.value==""){
	    //    document.write("No words entered!");
	    //    return;
	    // }

	   //console.log(chrome);
	   
		var currEmail = document.keywordform.email.value.toString();
		var currList = document.keywordform.listOfHandles.value.toString();
		var currType = document.keywordform.types.value.toString();
	 ...
	 //    fr.on("value", function(snapshot) {
		...
		fr.once("value", function(snap) {
			...
			for (var key in allUsers){
			  	var singleUser = allUsers[key];
			  	var uemail = singleUser["email"];
			  	...
			  	if (currEmail == uemail){
			  		...
			  		var finalJson = {
			  			...
			  			"list" : currList,
			  			"type" : currType,
			  			"toNotify": singleUser["toNotify"],
			  			"notif1": singleUser["notif1"],
			  			"notif2": singleUser["notif2"],
			  			"notif3": singleUser["notif3"],
			  			"notif4": singleUser["notif4"]
			  		};

			  		localStorage.setItem("currentUser", currEmail);

			  		var completeUserLocation = "https://tweettracker-3d814.firebaseio.com/Users/user-" + userNum.toString();

			  		console.log(finalJson);
			  		console.log(completeUserLocation);

			  		var fr2= new Firebase(completeUserLocation);
			  		fr2.set(finalJson);
	    ...
	    console.log("donepush");
	   	...
	   	//fr.set({'test1':s});
	   	//var re=fr.database().ref('user/'+message);
	    //fr.set({user:{userid:message,word:s}});
	    //fr.set({user:{[message] :s}});
	    //fr.push({word:s});
	   	//chrome.storage.sync.set({'words':document.keywordform.words.value}, function(){
	   	//document.write("Settings saved.");
